...
12. Run shadow trading on the same window to compare theoretical live decisions with observed orderbooks.
13. Run live readiness checks and review kill switch state.
14. Mine Obsidian Twitter-thread notes into Strategy Candidates when the notes contain real strategy content.
15. Review paper equity, shadow decisions, and readiness scores in the read-only dashboard.
...
- Compare paper fills with shadow fills before trusting a strategy candidate.
- Treat shadow-trading mismatches as evidence against an apparent paper edge.
...
Priorities:
- Add Prometheus metrics for ingestion, research signals, backtests, paper trading, shadow trading, and readiness checks.
...
- Add n8n alerts for high drawdown, low fill rate, data anomalies, shadow-vs-paper mismatch, degraded readiness, and kill switch triggers.
- Keep structured logs consistent across collectors, API jobs, paper trading, and evaluation jobs.
- Add a read-only self-hosted dashboard for paper equity, shadow decisions, readiness scores, PnL, exposure, fills, signals, stale data, and collector health.
...
- Feed WebSocket snapshots into shadow trading and readiness scoring.
...
- Shadow trading must show realistic fills, acceptable latency, low slippage, and no persistent paper-vs-shadow mismatch.
- Monitoring must catch data, fill, latency, and drawdown anomalies.
- Risk controls and kill switches must block invalid exposure, stale data, excessive order rate, excessive slippage, latency spikes, and abnormal market conditions.
